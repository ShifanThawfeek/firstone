            ...
            <v-form ref="form">
              ...
              >
                ...
                <v-radio
                  label="Image"
                  value="image"
                  @click="pageBackground = isUrl(customization.pageBackground) ? customization.pageBackground : 'https://resi.ze-robot.com/dl/4k/4k-desktop-wallpaper.-2560%C3%971440.jpg', type='image'"
                ></v-radio>
              </v-radio-group>
                ...
                <v-flex xs12 sm12 md12 lg12 xl12 class="px-5" v-if="type == 'color'">
                  <div class="my-3">
                    ...
                    <div v-if="pageBackground">
                      ...
                      <div>
                        <div :style="`display: inline-block; height: 25px; width: 25px;background:${pageBackground.colors[0] || '#F09819'}`"></div> 
                        <span>{{ pageBackground.colors[0] || '#F09819' }}</span>
                        <div :style="`display: inline-block; height: 25px; width: 25px;background:${pageBackground.colors[1] || '#EDDE5D'}`"></div>
                        <span>{{ pageBackground.colors[1] || '#EDDE5D' }}</span>
                      </div>
                      <div class="mb-3" :style="`padding: 100px; width: 100%;background: linear-gradient(45deg, ${pageBackground.colors[0] || '#F09819'}, ${pageBackground.colors[1] || '#EDDE5D'}`"></div>
                      <v-btn
                        @click="dialog = true"
                        color="primary"
                        outlined
              ...
              <!-- Font size & . . . -->
                <v-layout row wrap justify-center>
                  ...
                  <v-flex xs12 sm12 md12 lg6 xl6 class="px-5">
                    ...
                    <v-text-field  label="Buttons border size on hover" type="number" v-model="buttonBorderSizeHover"></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm12 md12 lg6 xl6 class="px-5">
                    <!-- <v-text-field :rules="[rules.required]" label="Buttons border color" v-model="buttonBorder"></v-text-field> -->
                    <v-text-field label="Buttons border color" v-model="buttonBorder"></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm12 md12 lg6 xl6 class="px-5">
                    <!-- <v-text-field :rules="[rules.required]" label="Buttons border color on hover" v-model="buttonBorderHover"></v-text-field> -->
                    <v-text-field label="Buttons border color on hover" v-model="buttonBorderHover"></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm12 md12 lg6 xl6 class="px-5">
                    <!-- <v-text-field :rules="[rules.required]" label="Shape of buttons" type="number" v-model="borderRadius"></v-text-field> -->
                    <v-text-field label="Shape of buttons" type="number" v-model="borderRadius"></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm12 md12 lg6 xl6 class="px-5">
                    <!-- <v-text-field :rules="[]" label="Icons colors" v-model="iconColor"></v-text-field> -->
                    <v-text-field label="Icons colors" v-model="iconColor"></v-text-field>
                  </v-flex>
                </v-layout>
              <!-- / Font size & . . . -->
              <div style="display: flex; justify-content: flex-end">
                <v-btn-toggle>
                  ...
                  <v-btn color="success accent-3" elevation="0" class="white--text mt-2" large @click="refreshPreview">Refresh</v-btn>
